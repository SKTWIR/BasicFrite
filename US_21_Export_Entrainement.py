# Fichier : US_21_Export_Entrainement.py
...
def load_user_entrainements(user_id):
    """Charge les entraînements de l'utilisateur en gérant l'encodage."""
    data = []
    if not os.path.exists(CSV_FILE):
        messagebox.showerror("Fichier Manquant", "Personne_Exo.csv introuvable.")
        return data

    try:
        # CORRECTION : Utilisation de 'utf-8-sig' pour gérer le BOM
        ...
        try:
            with open(CSV_FILE, newline='', encoding='latin-1') as f:
             ...
             messagebox.showerror("Erreur Lecture CSV", f"Impossible de lire Personne_Exo.csv: {e_latin}")
    return data
