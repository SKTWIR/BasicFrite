# Fichier : app_gui.py (Corrigé pour le retour des données)
...
def submit_data(return_callback, current_data): # <-- 1. Accepter les données ici
    """
    ...
    try:
        ...
        UserProfile(first_name, last_name, email, age_val, weight_val, height_val)

        # ... (Lecture et écriture du CSV) ...
        rows = []
        fieldnames = []
        found = False
        updated_row_data = current_data # Stocke les données mises à jour
            ...
            with open(USER_CSV_FILE, mode='r', newline='', encoding='utf-8') as f:
                ...
                for row in reader:
                    if row['id_user'] == current_editing_user_id:
                        ...
                        updated_row_data = row # Capture de la ligne mise à jour
                    rows.append(row)
        ...
        # --- CORRECTION (Bouton Enregistrer) ---
        # Passe les données mises à jour (updated_row_data) au callback
...
# La fonction doit accepter les données pour les retransmettre
...
def run_profile_screen(root_window, switch_to_menu_callback, user_data): 
    ...
    # ... (Configuration de la fenêtre et des widgets) ...
    for widget in root_window.winfo_children():
        widget.destroy()
    root_window.title("Profil Utilisateur")
    root_window.geometry("550x480") 
    ...
    # --- CORRECTION (Bouton Enregistrer) ---
    # Le lambda doit maintenant passer les 'user_data' à submit_data
    ...
    # --- CORRECTION (Bouton Retour) ---
    # Le lambda doit maintenant passer les 'user_data' à return_to_menu
...
if __name__ == '__main__':
    ...
    dummy_data = {'id_user': '1', 'prénom': 'Test', 'nom': 'User', 'email': 'test@test.com', 'age': '25', 'poids': '70', 'taille': '1.80'}
