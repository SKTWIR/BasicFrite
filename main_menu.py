...
def delete_account():
    ...
    confirm = messagebox.askyesno(
        ...
        f"ATTENTION: Êtes-vous sûr de vouloir supprimer définitivement votre compte '{user_pseudo}' ?\n\nCette action est irréversible."
    )
    ...
    if confirm:
        ...
        # 3. Renvoyer à l'écran de connexion (SANS CONFIRMATION)
...
def switch_to_login(force_logout=False):
    ...
    # Si la fenêtre n'existe pas (ex: après suppression), on lance l'app
    if not ('root' in globals() and root.winfo_exists()):
        run_app_start()
        return

    # Si force_logout est Vrai (après suppression), on ne demande pas
    if force_logout:
        connection_initial.run_connection_initial(root, switch_to_menu, switch_to_admin_menu)
    
    # Comportement normal (clic sur Déconnexion)
...
def switch_to_menu(user_data):
    ...
    tk.Button(
        root, text="💬 Notification", command=open_chat_window,
